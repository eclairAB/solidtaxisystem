...
<title>Taxi System - Cashier - PAG-IBIG</title>
		...
		<?php
			...
			if(isset($_SESSION['cashier_driver_id']))
			{
				...
				echo "
							...
							<h4><b>PAG-IBIG Remitance History</b></h4>
							...
							<table class='table table-striped' style='border:2px solid #5e78a3'>
								<thead style='color:white;background-color:#5e78a3' >
								    	...
								    	$row100 = MYSQL_SELECT("select remit,d_rem_date from driver_remitance where d_id = ". $_SESSION['cashier_driver_id'] . " and remitance_type='pagibig' order by d_rem_id desc",$sess_name100,1);
								    	...
								    	MYSQL_SELECT("select sum(billing_products.p_current_price) from billing_products,billing where billing.d_id = ". $_SESSION['cashier_driver_id'] . " and billing_products.b_id=billing.b_id and billing_products.p_current_name like 'pagibig%'",$sess_name101,0);
						...
						echo "
							...
							<form action='functions/cashier_pagibig_add.php' method='post'>
							...
							<center><button type='button' class='btn-success' style='padding:20px 40px 20px 40px' onclick='window.location.href=\"cashier_pagibig.php\"'>Go Back</button></center>
