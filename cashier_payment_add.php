<?php
		...
		if(isset($_POST['toreceipt']))
		{
		header("location:cashier.php"); // your current page
		}
		...
		if(ISLOGGEDIN()==true)
		{
			if(ISCASHIER()==true)
			{
				if(isset($_SESSION['cashier_driver_id']))
				{
					...
					if($_POST)
					{
						if(isset($_POST['enter_amount_tb']))
						{
							if($_POST['enter_amount_tb'] > 0)
							{
								$_SESSION['cashier_enter_amount'] = $_POST['enter_amount_tb'];
							}
							else
							{
			...
			if(isset($_SESSION['cashier_driver_id']))
			{
				...
				echo "
						...
						echo "
							...
							<form action='cashier_payment_receipt.php' method='post'>
							...
							<center><input type='submit' value='Confirm' name='toreceipt' class='btn-success' style='padding:20px 40px 20px 40px' /></center>
							...
							<center><button type='button' class='btn-success' style='padding:20px 40px 20px 40px' onclick='window.location.href=\"cashier_payment.php\"'>Go Back</button></center>
