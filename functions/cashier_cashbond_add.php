...
if($_POST)
{
	...
	if(isset($_POST['remitance_amount']))
	{
		...
		if(!is_numeric($_POST['remitance_amount']))
		{
			...
			header("location:../cashier_cashbond.php");
			$ok = false;
		}
		else
		{
			if($_POST['remitance_amount'] < 1)
			{
				...
				$ok = false;
								    	...
								    	$row100 = MYSQL_SELECT("select remit,d_rem_date from driver_remitance where d_id = ". $_SESSION['cashier_driver_id'] . " and remitance_type='cash bond' order by d_rem_id desc",$sess_name100,3);
		...
		if($ok == true)
		{
			...
			MYSQL_INSERT("driver_remitance","d_id,remit,d_rem_date,remitance_type",$_SESSION['cashier_driver_id'].",".$_POST['remitance_amount'].",'".Date("Y-m-d H:i:s")."','cash bond'");
