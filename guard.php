...
<body>
	...
	<?php include("includes/guard_nav.php"); PRINT_NAV(); ?>
	...
	<div id='body_div'>
		...
		else
		{
			...
			<form action='guard.php' method ='get'>
			...
			<form action='guard.php' method ='get'>
				...
				<center><input type='submit' value='Search' class='btn-success' style='padding:20px 40px 20px 40px' /></center>
		...
		if(isset($_SESSION['guard_taxi_id']))
		{
			echo "<hr>
		...
		<?php
				...
				if(preg_match("/^[0-9]{1,}$/", $_GET['driverid']))
				{
					...
					if(isset($_SESSION[$sess_name3.'_0_0']))
					{
						...
						else
						{
							...
							if(!isset($_SESSION['guard_driver_id']))
							{
								$_SESSION['guard_driver_id'] = $_GET['driverid'];
								header("location:guard.php");
							}
							else
							{
			...
			if(isset($_SESSION['guard_driver_id']))
			{
		    		...
		    		$row = MYSQL_SELECT("select * from driver where d_id=".$_SESSION['guard_driver_id'],$sess_name4,9);
					...
					if(isset($_SESSION[$sess_name8.'_0_0']))
					{
						...
						else
						{
							...
							if(!isset($_SESSION['guard_taxi_id']))
							{
								...
								$sess_name333 = "data333_" . Date('Y') . Date('m') . Date('d') . Date('H') . Date('i') . Date('s') ."1";
								MYSQL_SELECT("select out_time from in_out where t_id=".$_GET['taxiid']. " order by io_id desc limit 1",$sess_name333,0);

								$ok = true;

								if(isset($_SESSION[$sess_name333.'_0_0']))
								{
									if($_SESSION[$sess_name333.'_0_0']!="0000-00-00 00:00:00" && $_SESSION[$sess_name333.'_0_0'] != "")
									{
										$ok = false;
									}
								}
								...
								if($ok==true)
								{
									...
									$_SESSION['guard_taxi_id'] = $_GET['taxiid'];


									$sess_name88 = "data88_" . Date('Y') . Date('m') . Date('d') . Date('H') . Date('i') . Date('s');


									MYSQL_SELECT("select d_id,b_id,io_id from in_out where t_id=".$_SESSION['guard_taxi_id']." order by io_id desc limit 1",$sess_name88,2);
									...
									if(isset($_SESSION[$sess_name88.'_0_0']))
									{
										...
										{
											$_SESSION['guard_driver_id'] = $_SESSION[$sess_name88.'_0_0'];
											$_SESSION['guard_io_id'] = $_SESSION[$sess_name88.'_0_2'];
											header("location:guard.php");
										}
									}
									else
			...
			if(isset($_SESSION['guard_taxi_id']))
			{
		    		...
		    		$row3 = MYSQL_SELECT("select * from taxi where t_id=".$_SESSION['guard_taxi_id'],$sess_name9,4);
					 ...
					 if($row3 != 0)
					 {
				    	...
				    	for($x=0;$x<$row3;$x++)
				    	{
				    		...
				    		<td><a href='guard.php?taxiremove=".$_SESSION[$sess_name9.'_'.$x.'_0']."'><button type='button' id='_buttons' class='btn-danger'>Remove</button></a></td>
				    		</tr>";
				    	}
			...
			if(isset($_SESSION['guard_taxi_id']) && isset($_SESSION['guard_driver_id']))
			{
				...
				echo "
					...
					<center><button type='button' class='btn-success' style='padding:20px 40px 20px 40px' onclick='window.location.href=\"guard_in.php\"'>IN</button>
		 ...
		 if($row != 0)
		 {
		 	echo "
		    	...
		    	for($x=0;$x<$row;$x++)
		    	{
		    		...
		    		<td><a href='guard.php?driverid=".$_SESSION[$sess_name.'_'.$x.'_0']."'><button type='button' id='_buttons' class='btn-success'>Select Driver</button></a></td>
		    		</tr>";
		    	}
			...
			";
		 	...
		 	//search taxi
		    	...
		    	for($x=0;$x<$row2;$x++)
		    	{
								...
								MYSQL_SELECT("select out_time from in_out where t_id=".$_SESSION[$sess_name6.'_'.$x.'_0']. " order by io_id desc limit 1",$sess_name333,0);
								...
								if($ok==true)
								{
						    		...
						    		<td><a href='guard.php?taxiid=".$_SESSION[$sess_name6.'_'.$x.'_0']."'><button type='button' id='_buttons' class='btn-success'>Select Taxi</button></a></td>
						    		</tr>";
					    		}
		    	}
