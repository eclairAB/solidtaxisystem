...
<title>Taxi System - Remittance - Cash Bond</title>
		...
		<?php
			...
			if(isset($_SESSION['cashier_driver_id']))
			{
				...
				echo "
							...
							<h4><b>Cash Bond Remitance History</b></h4>
							...
							<table class='table table-striped' style='border:2px solid #5e78a3'>
								<thead style='color:white;background-color:#5e78a3' >
								    	...
								    	$row100 = MYSQL_SELECT("select remit,d_rem_date from driver_remitance where d_id = ". $_SESSION['cashier_driver_id'] . " and remitance_type='cash bond' order by d_rem_id desc",$sess_name100,1);
								    	...
								    	MYSQL_SELECT("select sum(billing_products.p_current_price) from billing_products,billing where billing.d_id = ". $_SESSION['cashier_driver_id'] . " and billing_products.b_id=billing.b_id and billing_products.p_current_name like 'cash bond%'",$sess_name101,0);
						...
						echo "
							...
							<form action='functions/remit_cashbond_add.php' method='post'>

							<div class='form-group'>
								<label id='search_label'>Remitance Amount</label>
								<input type='number' step='any' name='remitance_amount' id='search_box' class='form-control' value='".$_POST['remitance_amount']."' readonly/>
								<label class='big-text'>Remarks</label>
								<input type='text' name='remitance_remarks' id='search_box' class='form-control' value='".$_POST['remitance_remarks']."' required/>			
							</div>
							...
							<center><button type='button' class='btn-success' style='padding:20px 40px 20px 40px' onclick='window.location.href=\"remit_cash_bond.php\"'>Go Back</button></center>
